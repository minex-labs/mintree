...
const NONE: PriorityDisplay = { label: "", icon: " ", color: "gray" };
 ...
 */
export function priorityDisplay(priority: PriorityValue): PriorityDisplay {
	switch (priority) {
		case 1:
 ...
 */
export function prioritySortRank(priority: PriorityValue): number {
	if (priority == null || priority === 0) return Number.POSITIVE_INFINITY;
	return priority;
}
