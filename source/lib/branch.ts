...
export type ParsedBranch = {
	...
	// `type` and `desc` are only meaningful for convention branches
	// (`<type>/<issue>-<desc>`). Linear `branchName`-derived branches (see
	// `parseLinearBranch`) don't follow the convention, so both are absent
	// there — the branch carries Linear's own `<user>/<team>-<n>-<slug>` shape.
	type?: BranchType;
	desc?: string;
};
...
export function parseBranch(branch: string): ParsedBranch | ParseError {
...
export function isParseError(result: ParsedBranch | ParseError): result is ParseError {
 ...
 */
export function issueIdFromWorktreeDirName(dirName: string): string | null {
 ...
 */
export function extractLinearIssueId(branch: string, teamKeys: string[]): string | null {
 ...
 */
export function parseLinearBranch(
	rawBranch: string,
	teamKeys: string[],
): ParsedBranch | ParseError {
 ...
 */
export function isBareIssueIdBranch(parsed: ParsedBranch): boolean {
