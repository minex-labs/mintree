...
import { readMetadata, type ProviderKind } from "../metadata.js";
 ...
 */
export function createProvider(repoRoot: string): IssueProvider {
	const metadata = readMetadata(repoRoot);
	const kind: ProviderKind = metadata.provider ?? "github";
	switch (kind) {
		case "github":
 ...
 * single mapping.
 */
export function describeTransition(result: TransitionResult): {
	kind: "ok" | "skip" | "warn";
	label: string;
	detail?: string;
} {
