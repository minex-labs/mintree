 ...
 */
export function findMainRepoRoot(cwd: string = process.cwd()): string | null {
	try {
...
export function getMintreeDir(repoRoot: string): string {
	return path.join(repoRoot, ".mintree");
}

export function getMetadataPath(repoRoot: string): string {
...
export function getSessionStatesDir(repoRoot: string): string {
...
export function isGitIgnored(relativePath: string, cwd: string): boolean {
	try {
 ...
 */
export function isGitTracked(relativePath: string, cwd: string): boolean {
	try {
 ...
 */
export function findBranchConventionDoc(repoRoot: string): string | null {
	const candidates = [
 ...
 */
export function ensureGitignoreEntries(repoRoot: string, entries: string[]): string[] {
	...
	if (existing.length > 0) parts.push("\n");
	parts.push("# mintree\n");
...
export function getDefaultBranch(repoRoot: string): string | null {
	...
	for (const candidate of ["main", "master"]) {
		if (branchExists(repoRoot, candidate) !== null) return candidate;
	}
	return null;
}
...
export function branchExists(repoRoot: string, branch: string): BranchExistence {
	if (trySh(`git rev-parse --verify --quiet "refs/heads/${branch}"`, repoRoot)) return "local";
...
export type FetchResult = { ok: boolean; reason?: string };
 ...
 */
export function fetchRemote(repoRoot: string): FetchResult {
 ...
 */
export function worktreeForBranch(repoRoot: string, branch: string): string | null {
	...
	for (const block of blocks) {
		...
		for (const line of lines) {
			...
			if (line.startsWith("branch ")) wtBranch = line.slice("branch ".length);
		}
		if (wtPath && wtBranch === ref) return wtPath;
	}
	return null;
}
...
export function addWorktree(args: {
	...
	if (existence === "local") {
		...
		if (!base) throw new Error(`Cannot create new branch ${branch}: no base branch resolved.`);
 ...
 */
function trySh(cmd: string, cwd: string): string | null {
	try {
...
export function listWorktrees(repoRoot: string): WorktreeEntry[] {
	...
	for (const block of blocks) {
		...
		for (const line of block.split("\n")) {
			if (line.startsWith("worktree ")) entryPath = line.slice("worktree ".length);
...
export type AheadBehind = { ahead: number; behind: number; against: string };
 ...
 */
export function getAheadBehind(worktreePath: string, against?: string): AheadBehind | null {
	let ref = against;
	if (!ref) {
