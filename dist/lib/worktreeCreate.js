...
import { parseBranch, parseLinearBranch, isBareIssueIdBranch, isParseError, } from "./branch.js";
...
import { readMetadata, upsertIssue, setInitFailed } from "./metadata.js";
import { fetchIssueBranchName } from "./providers/linear.js";
/**
 ...
 */
function describeInitFailure(err) {
    const e = err;
    const asText = (v) => typeof v === "string" ? v.trim() : v ? v.toString("utf-8").trim() : "";
    ...
    if (tail.length > INIT_ERROR_MAX_CHARS)
        tail = `${tail.slice(0, INIT_ERROR_MAX_CHARS)}…`;
    return `${status}: ${tail}`;
}
 ...
 */
function materializeLinkFiles(repoRoot, worktreePath, linkFiles, pushStep) {
 ...
 */
async function bootstrapWorktree(root, worktreePath, issueId, pushStep, progress) {
...
export async function resolveBareIssueBranch(args) {
    ...
    const reason = found.kind === "unavailable"
        ? found.reason
...
export async function runCreate(branchArg, opts) {
    ...
    const linearTeamKeys = meta.provider === "linear" ? (meta.linear?.teams ?? []).map((t) => t.key) : [];
    ...
    await nextFrame(progress);
    const { initFailed, initError } = await bootstrapWorktree(root, worktreePath, parsed.issueId, pushStep, progress);
...
export async function runCreateDetached(opts) {
    ...
    await nextFrame(progress);
    const { initFailed, initError } = await bootstrapWorktree(root, worktreePath, opts.issueId, pushStep, progress);
    const work = opts.work && !initFailed;
    if (opts.work && initFailed) {
        pushStep({
            kind: "error",
